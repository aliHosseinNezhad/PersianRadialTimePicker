...
public class CustomNumberPicker extends LinearLayout {
    ...
    private float scrollVelocityWeight = 2.5f;
    ...
    public interface OnScrollListener {
         ...
         *                    {@link #SCROLL_STATE_IDLE}.
         */
        public void onScrollStateChange(CustomNumberPicker view, @ScrollState int scrollState);
    }
    ...
    public CustomNumberPicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        final TypedArray attributes = context.obtainStyledAttributes(attrs,
                R.styleable.CustomNumberPicker, defStyle, 0);

        final Drawable selectionDivider = attributes.getDrawable(
                R.styleable.CustomNumberPicker_np_divider);
        if (selectionDivider != null) {
            ...
            mDividerColor = attributes.getColor(R.styleable.CustomNumberPicker_np_dividerColor,
                    mDividerColor);
        ...
        maxTextLength = attributes.getInt(R.styleable.CustomNumberPicker_np_maxTextLength, 0);
        mDividerDistance = attributes.getDimensionPixelSize(
                R.styleable.CustomNumberPicker_np_dividerDistance, defDividerDistance);
        mDividerLength = attributes.getDimensionPixelSize(
                R.styleable.CustomNumberPicker_np_dividerLength, 0);
        mDividerThickness = attributes.getDimensionPixelSize(
                R.styleable.CustomNumberPicker_np_dividerThickness, defDividerThickness);
        mDividerType = attributes.getInt(R.styleable.CustomNumberPicker_np_dividerType, SIDE_LINES);

        mOrder = attributes.getInt(R.styleable.CustomNumberPicker_np_order, ASCENDING);
        mOrientation = attributes.getInt(R.styleable.CustomNumberPicker_np_orientation, VERTICAL);

        final float width = attributes.getDimensionPixelSize(R.styleable.CustomNumberPicker_np_width,
                SIZE_UNSPECIFIED);
        final float height = attributes.getDimensionPixelSize(R.styleable.CustomNumberPicker_np_height,
                SIZE_UNSPECIFIED);
        ...
        mValue = attributes.getInt(R.styleable.CustomNumberPicker_np_value, mValue);
        mMaxValue = attributes.getInt(R.styleable.CustomNumberPicker_np_max, mMaxValue);
        mMinValue = attributes.getInt(R.styleable.CustomNumberPicker_np_min, mMinValue);

        mSelectedTextAlign = attributes.getInt(R.styleable.CustomNumberPicker_np_selectedTextAlign,
                mSelectedTextAlign);
        mSelectedTextColor = attributes.getColor(R.styleable.CustomNumberPicker_np_selectedTextColor,
                mSelectedTextColor);
        mSelectedTextSize = attributes.getDimension(R.styleable.CustomNumberPicker_np_selectedTextSize,
                spToPx(mSelectedTextSize));
        mSelectedTextStrikeThru = attributes.getBoolean(
                R.styleable.CustomNumberPicker_np_selectedTextStrikeThru, mSelectedTextStrikeThru);
        mSelectedTextUnderline = attributes.getBoolean(
                R.styleable.CustomNumberPicker_np_selectedTextUnderline, mSelectedTextUnderline);
        mSelectedTypeface = Typeface.create(attributes.getString(
                R.styleable.CustomNumberPicker_np_selectedTypeface), Typeface.NORMAL);
        mTextAlign = attributes.getInt(R.styleable.CustomNumberPicker_np_textAlign, mTextAlign);
        mTextColor = attributes.getColor(R.styleable.CustomNumberPicker_np_textColor, mTextColor);
        mTextSize = attributes.getDimension(R.styleable.CustomNumberPicker_np_textSize,
                spToPx(mTextSize));
        mTextStrikeThru = attributes.getBoolean(
                R.styleable.CustomNumberPicker_np_textStrikeThru, mTextStrikeThru);
        mTextUnderline = attributes.getBoolean(
                R.styleable.CustomNumberPicker_np_textUnderline, mTextUnderline);
        mTypeface = Typeface.create(attributes.getString(R.styleable.CustomNumberPicker_np_typeface),
                Typeface.NORMAL);
        mFormatter = stringToFormatter(attributes.getString(R.styleable.CustomNumberPicker_np_formatter));
        mFadingEdgeEnabled = attributes.getBoolean(R.styleable.CustomNumberPicker_np_fadingEdgeEnabled,
                mFadingEdgeEnabled);
        mFadingEdgeStrength = attributes.getFloat(R.styleable.CustomNumberPicker_np_fadingEdgeStrength,
                mFadingEdgeStrength);
        mScrollerEnabled = attributes.getBoolean(R.styleable.CustomNumberPicker_np_scrollerEnabled,
                mScrollerEnabled);
        mWheelItemCount = attributes.getInt(R.styleable.CustomNumberPicker_np_wheelItemCount,
                mWheelItemCount);
        mLineSpacingMultiplier = attributes.getFloat(
                R.styleable.CustomNumberPicker_np_lineSpacingMultiplier, mLineSpacingMultiplier);
        mMaxFlingVelocityCoefficient = attributes.getInt(
                R.styleable.CustomNumberPicker_np_maxFlingVelocityCoefficient,
                mMaxFlingVelocityCoefficient);
        mHideWheelUntilFocused = attributes.getBoolean(
                R.styleable.CustomNumberPicker_np_hideWheelUntilFocused, false);
        mAccessibilityDescriptionEnabled = attributes.getBoolean(
                R.styleable.CustomNumberPicker_np_accessibilityDescriptionEnabled, true);
        mItemSpacing = attributes.getDimensionPixelSize(
                R.styleable.CustomNumberPicker_np_itemSpacing, 0);
        ...
        mWrapSelectorWheel = attributes.getBoolean(R.styleable.CustomNumberPicker_np_wrapSelectorWheel,
                mWrapSelectorWheel);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (isHorizontalMode()) {
                    int initialVelocity = (int) (velocityTracker.getXVelocity() * scrollVelocityWeight);
                    ...
                    int initialVelocity = (int) (velocityTracker.getYVelocity() * scrollVelocityWeight);
                    if (Math.abs(initialVelocity) > mMinimumFlingVelocity) {
                        fling(initialVelocity);
                        onScrollStateChange(OnScrollListener.SCROLL_STATE_FLING);
                    } else {
...
//        }
            ...
            if ((showSelectorWheel && i != mWheelMiddleItemIndex)
                ...
                if (autoTextSize)
                    if (getOrientation() == HORIZONTAL) {
                        int min = Math.min(getWidth() / getWheelItemCount(), getHeight());
                        mSelectorWheelPaint.setTextSize(min / (float) (maxTextLength == 0 ? scrollSelectorValue.length() : maxTextLength));
                    } else {
                        int min = Math.min(getWidth(), getHeight() / getWheelItemCount());
                        mSelectorWheelPaint.setTextSize(min / (float) (maxTextLength == 0 ? scrollSelectorValue.length() : maxTextLength));
                    }
    ...
    public void setScrollVelocityWeight(float weight) {
        scrollVelocityWeight = weight;
    }
    ...
    public void setAutoTextSize(Boolean autoTextSize, int maxTextLength) {
